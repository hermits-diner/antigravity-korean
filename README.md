...
5. **한국어 & 영문 명칭 병기 (Bilingual Notation) 완벽 지원**
   - 설정 메뉴, 사이드바, 버튼, 단축키 등에 `한국어 (English)` 형식(예: `일반설정 (General)`, `앱설정 (Application)`, `단축키 (Shortcuts)`, `사이드바 (Toggle Sidebar)`)으로 병기 표기하여 공식 영문 문서나 해외 커뮤니티 팁을 참조할 때도 혼동 없이 편리하게 사용할 수 있습니다.

6. **AI 에이전트 한국어 전역 규칙(Global Rules) 자동 설정**
...
2. 원하는 언어 모드를 선택합니다:
   - **`[1] 순수 한글 모드 (Pure Korean)`**: 깔끔한 한글 전용 UI (일반 사용자 권장)
   - **`[2] 한영 병기 모드 (Bilingual)`**: 한국어 및 영문 명칭 동시 표기 (예: `일반설정 (General)`, `앱설정 (Application)`)
   - **`[3] 영문 원본 복원 (Restore Original)`**: 패치를 제거하고 원래의 영문 상태로 완벽 복원

### 3. 언어 설정 변경 방법 (4가지 지원)
설치 후 언제든 원하는 방식으로 모드를 자유롭게 전환할 수 있습니다:
- **방법 1 (화면 위젯)**: 앱 우측 하단의 **`[🌐 한영 병기 ▾]`** 버튼을 클릭하여 원하는 언어 모드로 즉시 전환
- **방법 2 (단축키)**: 키보드 **`Ctrl + Shift + L`**을 누르면 [한영 병기] ↔ [순수 한글] ↔ [영문 원본]으로 즉시 순환 변경
- **방법 3 (메뉴바)**: **`Alt`** 키를 누르면 상단 메뉴바가 나타나며, **`언어 (Language)`** 메뉴에서 원하는 모드 선택
- **방법 4 (재설치)**: `install.bat`을 다시 실행하여 원하는 모드(1번 또는 2번)를 선택

### 4. 삭제 및 원본 복구
- 패치를 삭제하고 최초 영문 상태로 되돌리려면 `uninstall.bat`을 실행하거나, `install.bat`에서 `3`을 선택하면 됩니다.
